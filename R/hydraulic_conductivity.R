...
#' \deqn{K(h) = K_s \cdot S_e^{0.5} \cdot \left(1 - \left(1 - S_e^{1/m}\right)^m\right)^2}
#'
...
#'
#' @export
hydraulic_conductivity <- function(data, ks, alpha, n, m, h) {
  ...
  # Effective saturation — scalars recycle; no rep() needed
  Se <- 1 / (1 + (.alpha * abs(.h))^.n)^.m

  # Mualem-Van Genuchten K(h) — direct tibble assignment
  out     <- tibble::as_tibble(data)
  out$.K  <- .ks * Se^0.5 * (1 - (1 - Se^(1 / .m))^.m)^2
  out
}
